...
* __physical_distance:__ An array of matrices, each matrix shows distance between each pair of nodes for a species. The diagonal will be set to zero. The rows and columns of the matrix are node numbers in order. Values are read column-wise: each column shows out-migration to all other nodes from a node. If instead of a matrix, there is `nothing`, no migration occurs for that species.
* __migration_trait:__ An tuple of type `Union{Int, Nothing}` where each element is the index of the migration trait (an index among `phenotypes`). If `nothing`, the species does not migrate. 
